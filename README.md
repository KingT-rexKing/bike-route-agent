...
### フルスクリーン演出 & インタラクティブUI

- **実写真スライドショー背景** — Unsplashの実写真を Ken Burns ズーム + クロスフェードで自動切替（ロード失敗時はグラデーションに自動フォールバック）
- **フルスクリーン特効** — Canvas パーティクル（火の粉・スピードライン）、マウス追従パララックス & グロー
- **クリック可能なスライドインジケーター** — 画面右下のドットで背景写真を手動切替
- **人気スポットのワンクリック入力** — 箱根・奥多摩・秩父などをボタンで即セット
- **時間帯レンジスライダー** — 出発〜帰着をドラッグで直感設定
- **リアルタイム進捗表示** — `st.status` でAI解析の各ステップをライブ表示、完了時はお祝い演出
